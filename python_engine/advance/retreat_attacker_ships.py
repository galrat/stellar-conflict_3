...
def get_retreat_valid_areas_attacker_ships(state, tile_key, contested_idx, loser):
    ...
    print(f"[RETREAT ATTACKER SHIPS] source_areas={source_areas}")
    print(f"[RETREAT ATTACKER SHIPS] friendly={friendly}, neutral={neutral}")

    return {'friendly': friendly, 'neutral': neutral}


def retreat_attacker_ships(state, player_id, retreat_tile_key, retreat_area_idx):
    ...
    state.setdefault('log', []).append({
        'message': f'{loser_name} (нападавший, корабли) отступил в {retreat_tile_key}:{retreat_area_idx} ({len(loser_units)} юн.). Статус: разбит.',
        'player_id': player_id,
    })
