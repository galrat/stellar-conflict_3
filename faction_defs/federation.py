...
FEDERATION = Faction(
    ...
    flavor = (
        ...
        "Федерация делает ставку на дипломатию и адаптивность: "
        "её армия хорошо скоординирована, а командиры умеют отступать "
        "без потерь и возвращаться вдвойне сильнее."
    ),
    special_ability_name = "Дипломатический иммунитет",
    special_ability_desc = (
        "Один раз за партию Федерация может отклонить вражескую атаку: "
        "атакующий теряет приказ, но не получает зону без боя. "
        "Применяется до розыгрыша боевых карт."
    ),
